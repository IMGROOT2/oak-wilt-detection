...
def main():
    ...
    # Note: We are now using the "Progressive Model" which expects:
    # ['log_duration', 'log_points', 'regional_precip', 'regional_humidity', 'regional_wind']
    # We map the enriched columns to these expected feature names.
    
    enriched_df['log_duration'] = np.log1p(enriched_df['duration_days'])
    enriched_df['log_points'] = np.log1p(enriched_df['point_count'])
    enriched_df['regional_precip'] = enriched_df['avg_precip']
    enriched_df['regional_humidity'] = enriched_df['avg_humidity']
    enriched_df['regional_wind'] = enriched_df['avg_wind']
    ...
    for cluster in clusters_data['clusters']:
        ...
        if c_id in enriched_lookup:
            ...
            # Add Patient Zero
            cluster['patient_zero'] = {
                'latitude': float(data['patient_zero_lat']),
                'longitude': float(data['patient_zero_lon']),
                'date': data['patient_zero_date']
            }
